...
class InventoryOptimizer:
    ...
    def _validate_inputs(
        self, forecast_df: pd.DataFrame, costs_cfg: CostsConfig
    ) -> None:
        """Validate input data and configuration."""
        required_columns = ["sku", "units_sold", "price", "on_hand"]
        missing_columns = [
            col for col in required_columns if col not in forecast_df.columns
        ]
        if missing_columns:
    ...
    def _create_decision_variables(self, skus: List[str]) -> None:
        ...
        self.reorder_variables = LpVariable.dicts(
            "ReorderQuantity", skus, lowBound=0, cat="Integer"
        )

        # Clearance discount variables (continuous, 0 <= x <= max_discount)
        self.discount_variables = LpVariable.dicts(
            "ClearanceDiscount", skus, lowBound=0, upBound=1.0, cat="Continuous"
        )
    ...
    def _add_shelf_space_constraint(
        self, forecast_df: pd.DataFrame, costs_cfg: CostsConfig
    ) -> None:
    ...
    def _add_service_level_constraints(
        self, forecast_df: pd.DataFrame, costs_cfg: CostsConfig
    ) -> None:
        ...
        for _, row in forecast_df.iterrows():
            ...
            self.problem += (
                current_inventory + self.reorder_variables[sku]
            ) >= required_stock

        logger.info("Service level constraints added")

    def _add_discount_constraints(
        self, forecast_df: pd.DataFrame, costs_cfg: CostsConfig
    ) -> None:
    ...
    def optimize_inventory(
        ...
        try:
            ...
            if status != "Optimal":
                logger.warning(
                    f"Optimization did not find optimal solution. Status: {status}"
                )
                return {
                    "status": status,
                    "orders": {},
            ...
            # Extract results
            orders = {sku: int(self.reorder_variables[sku].varValue) for sku in skus}

            discount_schedule = {
                sku: float(self.discount_variables[sku].varValue) for sku in skus
            }
            ...
            results = {
                ...
                "metrics": self._calculate_metrics(
                    forecast_df, orders, discount_schedule, costs_cfg
                ),
            }

            logger.info(
                f"Optimization completed successfully. Total cost: {total_cost}"
            )
            return results
