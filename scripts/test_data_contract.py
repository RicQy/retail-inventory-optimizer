...
from app.models.retail_sales import RetailSalesBatch, RetailSalesRecord
...
def test_data_contract():
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if idx < 3:  # Show first 3 records
                print(
                    f"✓ Record {idx + 1}: {record.store_id}, {record.sku}, {record.date}"
                )
    ...
    try:
        batch = RetailSalesBatch(records=valid_records, batch_id="test_batch_001")
