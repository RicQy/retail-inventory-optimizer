...
from app.etl.validation import DataValidator, DataQualityMetrics, RetailDataSchema
...
class TestDataValidation:
    ...
    def _create_invalid_data(self) -> pd.DataFrame:
        ...
        data = {
            ...
            'product_category': ['Electronics', 'InvalidCategory', 'Food']  # Invalid category
        }
        return pd.DataFrame(data)
    
    @patch('boto3.client')
    def test_data_quality_metrics_calculation(self, mock_boto3):
    ...
    @patch('boto3.client')
    def test_pandera_validation(self, mock_boto3):
        ...
        result = validator.validate_with_pandera(
            self.sample_data, 
            validator.schemas.RETAIL_SALES_SCHEMA
        )
    ...
    @patch('boto3.client')
    def test_schema_drift_detection(self, mock_boto3):
    ...
    @patch('boto3.client')
    def test_comprehensive_validation(self, mock_boto3):
...
class TestETLPipeline:
    ...
    @patch('boto3.client')
    def test_etl_pipeline_initialization(self, mock_boto3):
        """Test ETL pipeline initialization"""
        mock_s3 = MagicMock()
        mock_cloudwatch = MagicMock()
        mock_s3_validator = MagicMock()
        mock_cloudwatch_metrics = MagicMock()
        mock_boto3.side_effect = [mock_s3, mock_cloudwatch, mock_s3_validator, mock_cloudwatch_metrics]
...
class TestIntegration:
    ...
    @patch('boto3.client')
    def test_end_to_end_validation(self, mock_boto3):
        ...
        transform_validation = pipeline.validate_transformed_data(transformed_df)
    ...
    @patch('boto3.client')
    def test_failure_handling(self, mock_boto3):
        ...
        assert 'validation_failed' in validation_result or validation_result.get('overall_success') == False
