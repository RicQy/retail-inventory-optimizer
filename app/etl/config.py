...
class ETLConfig:
    ...
    VALIDATION_BUCKET = os.getenv(
        "ETL_VALIDATION_BUCKET", "retail-data-validation-reports"
    )
    ...
    FAIL_FAST_ON_VALIDATION = (
        os.getenv("FAIL_FAST_ON_VALIDATION", "true").lower() == "true"
    )
    ENABLE_SCHEMA_DRIFT_DETECTION = (
        os.getenv("ENABLE_SCHEMA_DRIFT_DETECTION", "true").lower() == "true"
    )

    # CloudWatch Configuration
    CLOUDWATCH_NAMESPACE_ETL = os.getenv("CLOUDWATCH_NAMESPACE_ETL", "ETL/Pipeline")
    CLOUDWATCH_NAMESPACE_QUALITY = os.getenv(
        "CLOUDWATCH_NAMESPACE_QUALITY", "ETL/DataQuality"
    )
    ENABLE_CLOUDWATCH_METRICS = (
        os.getenv("ENABLE_CLOUDWATCH_METRICS", "true").lower() == "true"
    )
    ...
    MAX_DUPLICATE_RATIO = float(
        os.getenv("MAX_DUPLICATE_RATIO", "0.05")
    )  # 5% max duplicates
