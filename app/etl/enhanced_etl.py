...
from .validation import DataValidator, setup_logging, DataValidationError, SchemaValidationError
...
class ETLPipeline:
    ...
    def __init__(self, 
        ...
        if self.enable_validation:
            self.validator = DataValidator(
                ge_data_context_root=config.get('ge_context_root', 'app/etl/ge_data_context'),
                s3_bucket=config.get('validation_bucket', 'retail-data-validation-reports'),
        ...
        try:
            ...
            for metric_name, value in metrics.items():
                metric_data.append({
                    ...
                    'Value': float(value),
                    'Unit': 'Count' if 'count' in metric_name.lower() else 'Seconds' if 'time' in metric_name.lower() else 'None',
                    'Dimensions': [
    ...
    def validate_raw_data(self, df: pd.DataFrame, dataset_name: str = "raw_data") -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                'validation_failures': 0 if validation_results['overall_success'] else 1,
    ...
    def transform_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            initial_rows = len(df)
            df = df.dropna(subset=['date', 'sales', 'items', 'store_id', 'product_category'])
            ...
            df_grouped = df.groupby(['date', 'store_id', 'product_category']).agg({
                'sales': 'sum',
                'items': 'sum',
                'year': 'first',
                'month': 'first', 
            ...
            try:
                ...
                logger.warning(f"Calendar completion failed: {str(e)}, using grouped data")
                df_grouped.reset_index(inplace=True)
                df = df_grouped
    ...
    def validate_transformed_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results = self.validator.validate_with_pandera(
                df, 
                self.validator.schemas.PROCESSED_DATA_SCHEMA
            )
            ...
            metrics = {
                ...
                'post_transform_validation_success': 1 if validation_results['passed'] else 0,
                'post_transform_validation_failures': 0 if validation_results['passed'] else 1,
            }
            ...
            logger.info("Transformed data validation completed", 
            ...
            logger.error("Transformed data validation failed", error=str(e), stage=stage)
    ...
    def save_data(self, df: pd.DataFrame, output_path: str, s3_key: str) -> Dict[str, Any]:
    ...
    def run_pipeline(self, data_url: str, output_path: str, s3_key: str) -> Dict[str, Any]:
