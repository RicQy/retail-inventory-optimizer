...
class JobOrchestrator:
    ...
    async def create_job(self, job_type: JobType, input_data: Dict[str, Any], 
        ...
        try:
            ...
            if not success:
                ...
                await self.aws_manager.update_job_metadata(
                    job_id, 
                    {"status": JobStatus.FAILED.value, "error_message": "Failed to enqueue job"}
                )
    ...
    async def process_job(self, job_id: str) -> JobResult:
        ...
        try:
            ...
            await self.aws_manager.update_job_metadata(
                job_id, 
                {"status": JobStatus.RUNNING.value, "progress": 0.1}
            )
                ...
                try:
                    ...
                    if lambda_result:
                        ...
                        return JobResult(
                            ...
                            output_data={"lambda_request_id": lambda_result.get("request_id")}
                        )
                    else:
            ...
            if job_metadata.job_type in self.local_processors:
                ...
                raise Exception(f"No processor available for job type: {job_metadata.job_type}")
    ...
    async def retry_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if job_metadata.status != JobStatus.FAILED:
                logger.warning(f"Job {job_id} cannot be retried (status: {job_metadata.status})")
                return False
            ...
            if success:
                logger.info(f"Job {job_id} retried (attempt {job_metadata.retry_count + 1})")
    ...
    async def get_job_metrics(self, job_id: str) -> Dict[str, Any]:
        ...
        if job_metadata.status == JobStatus.COMPLETED:
            execution_time = (job_metadata.updated_at - job_metadata.created_at).total_seconds()
...
job_orchestrator.register_local_processor(JobType.OPTIMIZATION, process_optimization_job)
